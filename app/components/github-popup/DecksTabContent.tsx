...
function loadDeckFromStorage(id: string): Deck | null {
  try {
    const stored = localStorage.getItem(`${STORAGE_KEY}-deck-${id}`);
...
export default function DecksTabContent() {
  ...
  const downloadDeck = useCallback((deck: Deck) => {
    const blob = new Blob([deck.toJSONString()], { type: 'application/json' });
  ...
  const uploadDeck = useCallback(async (deck: Deck) => {
    if (!configured) { toast.error('Configure GitHub first'); return; }
    setUploadingId(deck.id);
    try {
      const sha = await getFileSha(config as GitHubConfig, `assets/decks/${deck.id}.json`);
      if (sha) toast.warning(`${deck.title} already exists on GitHub — updating.`);
      const result = await uploadFile(config as GitHubConfig, { path: `assets/decks/${deck.id}.json`, content: deck.toJSONString() }, `Update deck: ${deck.title}`);
      if (result.success) {
        toast.success(`Uploaded & removed: ${deck.title}`);
        localStorage.removeItem(`${STORAGE_KEY}-deck-${deck.id}`);
    ...
    setUploadingId(null);
  }, [config, configured, toast]);

  const handleImport = useCallback(() => {
    if (!importText.trim()) { toast.warning('Paste JSON first'); return; }
    try {
      const parsed = JSON.parse(importText);
      if (parsed.id && (parsed.cards || parsed.title)) {
        const deck = Deck.fromJSON(parsed);
        if (deck.id) {
          localStorage.setItem(`${STORAGE_KEY}-deck-${deck.id}`, deck.toJSONString());
          toast.success(`Imported: ${deck.title}`);
          setImportText('');
        } else {
          toast.error('Invalid deck: missing id');
        }
      } else {
        toast.error('Invalid format: expected {id, title, cards: [...]}');
      }
  ...
  const deleteDeck = useCallback((id: string) => {
    ...
    localStorage.removeItem(`${STORAGE_KEY}-deck-${id}`);
  ...
  return (
    <div className="space-y-3 pt-2">
      <div className="flex items-center justify-between">
        <span className="text-sm text-[var(--color-text)]">Decks</span>
        <span className="text-xs text-[var(--color-text-muted)]">{deckMetas.length} decks</span>
      </div>
      ...
      <Tabs defaultValue="decks">
        ...
        <TabsContent value="decks">
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {allDecks.length === 0 ? (
              <p className="text-sm text-[var(--color-text-muted)] text-center py-4">No decks</p>
            ) : (
              allDecks.map(deck => (
                <div key={deck.id} className="flex items-center gap-2 p-2 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)]">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[var(--color-text-muted)]">{deck.getAllCards().length} cards</p>
                  </div>
          ...
          {configured && allDecks.length > 0 && (
            <p className="text-xs text-[var(--color-text-muted)] mt-2">Path: <code className="text-[var(--color-primary)]">assets/decks/[deck-id].json</code></p>
          )}
        </TabsContent>

        <TabsContent value="import">
          <div className="space-y-2">
            <p className="text-xs text-[var(--color-text-muted)]">Paste a single deck JSON</p>
            <Input label="Paste JSON" multiline rows={4} value={importText} onChange={(e) => setImportText((e.target as HTMLTextAreaElement).value)} placeholder='{"id": "deck-1", "title": "...", "cards": [...]}' />
